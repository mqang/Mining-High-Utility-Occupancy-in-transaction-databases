...
# ==============================================================================
# 4. HÀM TIỀN XỬ LÝ DỮ LIỆU MUSHROOM
# ==============================================================================
def preprocess_mushroom_data(spark, file_path):
    """Đọc và chuyển đổi dữ liệu mushroom từ định dạng đặc biệt."""
    ...
    df = df.withColumn("Quantity", lit(1)) \
           .withColumn("UnitPrice", (rand(seed=42) * 9 + 1).cast("double")) # UnitPrice cần cho HUOMIL
...
def run_huomil(conf, file_path, minUtil, minUO_1item, minUO):
    ...
    print("\n" + "="*20 + " Starting HUOMIL Application (mushroom) " + "="*20)
    spark = SparkSession.builder.config(conf=conf.setAppName("mushroom_HUOMIL_App")).getOrCreate()
    ...
    # Đọc và tiền xử lý dữ liệu mushroom
    huomil_df = preprocess_mushroom_data(spark, file_path)
    huomil_df.cache()
...
def run_huopm(conf, file_path, minUtil, minUO):
    ...
    print("\n" + "="*20 + " Starting HUOPM Application (mushroom) " + "="*20)
    spark = SparkSession.builder.config(conf=conf.setAppName("mushroom_HUOPM_App")).getOrCreate()
    sc = spark.sparkContext; app_id = sc.applicationId; print(f"HUOPM running with Application ID: {app_id}")
    process = psutil.Process(os.getpid()); start_time = time.time(); cpu_start = process.cpu_times()

    huopm_df = preprocess_mushroom_data(spark, file_path)
    huopm_df.cache()
...
def main():
    ...
    # Tạo đường dẫn file và các ngưỡng cho tập dữ liệu mushroom
    file_path = "hdfs://localhost:9000/user/mqang/input/mushroom_utility_SPMF.txt"
    minUtil = 400
    minUO_1item = 0.001
    minUO = 0.001

    print("="*20 + " Calculating Initial mushroom Data Statistics " + "="*20)
    spark_stats = SparkSession.builder.config(conf=conf.setAppName("mushroom_Stats_App")).getOrCreate(); spark_stats.sparkContext.setLogLevel("WARN")
    try:
        df_stats = preprocess_mushroom_data(spark_stats, file_path)
        print("\nTransaction Utility (TU) Statistics for mushroom (from data):"); print(df_stats.select("TU").describe().toPandas())
    finally:
        spark_stats.stop(); print("="*20 + " Finished Initial mushroom Data Statistics " + "="*20)
    ...
    print("\n\n" + "="*20 + " FINAL mushroom COMPARISON " + "="*20); print(f"Jaccard Similarity between 2-itemsets: {jaccard_similarity:.4f}\n")
    ...
    fig, axs = plt.subplots(2, 3, figsize=(18, 10)); fig.suptitle("HUOMIL vs HUOPM Performance Comparison on mushroom Dataset", fontsize=16)
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96]); output_image_path = "mushroom_performance_comparison.png"; plt.savefig(output_image_path); print(f"\nBiểu đồ đã được lưu thành công vào file: {output_image_path}")
