...
from pyspark.sql.functions import col, avg, sum as spark_sum, count, concat_ws, lit, rand, monotonically_increasing_id
...
# ==============================================================================
# 4. HÀM TIỀN XỬ LÝ DỮ LIỆU KOSARAK
# ==============================================================================
def preprocess_kosarak_data(spark, file_path):
    df = spark.read.text(file_path)
    df = df.selectExpr("split(value, ' ') as items").withColumn("InvoiceNo", monotonically_increasing_id())
    df = df.selectExpr("InvoiceNo", "explode(items) as StockCode")
    df = df.withColumn("Quantity", lit(1)).withColumn("UnitPrice", (rand(seed=42) * 9 + 1).cast("double"))
    df = df.withColumn("CustomerID", lit("CUST_KOSARAK"))
...
def run_huomil(conf, file_path, minUtil, minUO_1item, minUO):
    ...
    print("\n" + "="*20 + " Starting HUOMIL Application (Kosarak) " + "="*20)
    spark = SparkSession.builder.config(conf=conf.setAppName("Kosarak_HUOMIL_App")).getOrCreate()
    ...
    clean_df = preprocess_kosarak_data(spark, file_path); clean_df.cache()
    ...
    print(f"HUOMIL Transactions with TU >= {minUtil}: {huomil_df.select('InvoiceNo').distinct().count()}")
    
    uolist_df = huomil_df.select("StockCode", "InvoiceNo", "item_util", "TU") \
                         .groupBy("StockCode", "InvoiceNo", "TU").agg(spark_sum("item_util").alias("IU"))
    uolist_df.cache()
    
    print("\n====== HUOMIL: VALID ITEMS in UOList ======")
    uolist_df.show() # In ra uolist trước khi lọc 1-itemset

    candidate_1item = uolist_df.groupBy("StockCode").agg(
...
def run_huopm(conf, file_path, minUtil, minUO):
    ...
    print("\n" + "="*20 + " Starting HUOPM Application (Kosarak) " + "="*20)
    spark = SparkSession.builder.config(conf=conf.setAppName("Kosarak_HUOPM_App")).getOrCreate()
    sc = spark.sparkContext; app_id = sc.applicationId
    ...
    print(f"HUOPM Transactions with TU >= {minUtil}: {huopm_df.select('InvoiceNo').distinct().count()}")

    uolist_df = huopm_df.select("StockCode", "InvoiceNo", "item_util", "TU") \
                        .groupBy("StockCode", "InvoiceNo", "TU").agg(spark_sum("item_util").alias("IU"))
    uolist_df.cache()

    results_1item = uolist_df.groupBy("StockCode").agg(
        spark_sum("IU").alias("total_util"),
        (spark_sum(col("IU") / col("TU")) / count("*")).alias("avg_uo")
    ).filter((col("total_util") >= minUtil) & (col("avg_uo") >= minUO))
...
def main():
    ...
    file_path = "hdfs://localhost:9000/user/mqang/input/kosarak.txt"
    minUtil = 15; minUO_1item = 0.15; minUO = 0.15

    print("="*20 + " Calculating Initial Kosarak Data Statistics " + "="*20)
    spark_stats = SparkSession.builder.config(conf=conf.setAppName("Kosarak_Stats_App")).getOrCreate()
    spark_stats.sparkContext.setLogLevel("WARN")
    try:
        clean_df_stats = preprocess_kosarak_data(spark_stats, file_path)
        tu_df_stats = clean_df_stats.groupBy("InvoiceNo").agg((spark_sum(col("Quantity") * col("UnitPrice"))).alias("TU"))
        print("\nTransaction Utility (TU) Statistics for Kosarak:"); print(tu_df_stats.select("TU").describe().toPandas())
    finally:
        spark_stats.stop(); print("="*20 + " Finished Initial Kosarak Data Statistics " + "="*20)
    ...
    print("\n\n" + "="*20 + " FINAL KOSARAK COMPARISON " + "="*20)
    ...
    fig.suptitle("HUOMIL vs HUOPM Performance Comparison on Kosarak Dataset", fontsize=16)
    ...
    output_image_path = "kosarak_performance_comparison.png"
